...
// Connexion à SQLite
const db = new sqlite3.Database('./db/database.sqlite', sqlite3.OPEN_READONLY, (err: { message: any; }) => {
  if (err) {
...
app.post('/api/login', (req: { body: { username: any; password: any; }; headers: { [x: string]: any; }; }, res: { status: (arg0: number) => { (): any; new(): any; send: { (arg0: string): any; new(): any; }; }; send: (arg0: string) => void; }) => {
  ...
  db.get(query, (err: { message: string; }, user: { username: string; }) => {
    if (err) {
