...
app.post("/api/pokemon", async (req: any, res: any) => {
    ...
    try {
        ...
        db.get(query, [pokemon_name], (err: any, row: any) => {
            if (err) {
...
app.post("/api/pokemons", (req: any, res: any) => {
    ...
    query += " WHERE ";
    if (filter) {
        query += `name LIKE '${filter}%' AND `;   
    }
    
    query += " id != '9425013436722359540686860091839895206901157191498492454862042052929570854061024990149995851733919624829' ORDER BY name ASC";
